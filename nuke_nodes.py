...
LOG_FILE = '/var/log/asterisk/disconnects.log'
...
def log_disconnect(node_id):
    """Logs the disconnection event to the log file."""
    if not os.path.exists(os.path.dirname(LOG_FILE)):
        os.makedirs(os.path.dirname(LOG_FILE))
    with open(LOG_FILE, 'a') as log_file:
        log_file.write(f"{time.strftime('%Y-%m-%d %H:%M:%S')} - Disconnected node ID: {node_id} from {args.initial_node_id}\n")

def run_command(node_id):
    """Runs the system command with the given node ID and logs the disconnection."""
    command = f"asterisk -rx 'rpt fun {args.initial_node_id} *1{node_id}'"
    if not args.quiet:
        print(f"Running command: {command}")
    subprocess.run(command, shell=True)
    log_disconnect(node_id)
